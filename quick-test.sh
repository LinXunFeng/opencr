...
echo "            （异步执行，返回 202 与 run_uid；进度见后台或下面的接口）"
echo "  查看进度: curl -H 'X-Admin-Token: <token>' \\"
echo "            http://localhost:9034/api/admin/runs/<run_uid>"
echo "  后台面板: http://localhost:9034/admin?token=<token>"
echo "            （需在 config.yaml 中设置 admin.enabled=true 与 admin.token）"
echo -e "${BLUE}================================${NC}"
