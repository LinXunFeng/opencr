...
def _apply_sqlite_pragmas(dbapi_connection, _connection_record) -> None:
    ...
    try:
        # busy_timeout 必须第一个设：切换 journal_mode 需要独占锁，
        # 多个 worker 同时启动时会撞在一起。超时未先设好的话，
        # 后到的连接会立刻抛 "database is locked" 而不是等待。
        cursor.execute("PRAGMA busy_timeout=5000")
        try:
            cursor.execute("PRAGMA journal_mode=WAL")
        except Exception as e:
            # WAL 是持久化在数据库文件上的设置，一次设成即长期有效。
            # 这里设不上通常意味着别的连接刚设过，不值得让服务起不来。
            logger.warning("Failed to switch journal_mode to WAL: %s", e)
