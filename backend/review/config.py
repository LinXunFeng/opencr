...
def _resolve_config_candidates() -> list:
    # 当前文件位于 backend/review/config.py，module_dir 指向 backend
...
def resolve_active_config_path() -> str:
    """
    返回实际被加载的 config.yaml 路径；找不到时返回空串。

    密码哈希化需要就地改写这个文件，因此不能只拿到解析后的字典。
    """
...
def get_app_version() -> str:
    """读取服务版本：固定从 backend/__init__.py 获取"""
...
def _resolve_default_data_dir() -> Path:
    ...
    # backend/review/config.py -> backend -> 项目根
...
def load_admin_config() -> dict:
    ...
    启用时 username/password 必填；缺失时由调用方拒绝启动，而不是静默放行。

    password 既可能是用户手写的明文，也可能是首次启动后被自动替换的哈希，
    判定交给 admin.auth.is_hashed()，这里只负责原样读出来。
    """
    config_data = load_file_config()

    enabled_raw = _pick_config_value(config_data, "admin.enabled", "OPENCR_ADMIN_ENABLED")
    username = _pick_config_value(config_data, "admin.username", "OPENCR_ADMIN_USERNAME")
    password = _pick_config_value(config_data, "admin.password", "OPENCR_ADMIN_PASSWORD")
    ...
    env_username = os.getenv("OPENCR_ADMIN_USERNAME", "").strip()
    env_password = os.getenv("OPENCR_ADMIN_PASSWORD", "").strip()
    ...
    resolved = {
        "enabled": _truthy(enabled_raw, False),
        "username": username or "admin",
        "password": password,
        "bind_local_only": _truthy(bind_local_only_raw, False),
    }
    # password 无论明文还是哈希都不进日志：哈希泄露等于打开离线爆破的门
    logger.info(
        "Admin config resolved: enabled=%s, username=%s, password=%s, bind_local_only=%s",
        resolved["enabled"],
        resolved["username"],
        "<set>" if resolved["password"] else "<empty>",
