...
5. 按关注点读取真实源码取证，产出逐条发现
6. 与上一次的结果比对，报告分「新增 / 仍存在 / 已消失」呈现
...
巡检每次分析的都是**全量代码**，所以逐轮产出高度重合：第一次跑出 80 条，第二周还是那 80 条加上新增的几条。报告因此分三段呈现，默认停在「新增」：
...
觉得某条问题不用再提，可以标记为「不再提醒」，后续巡检不会再产出它。
...
- **单次巡检有硬预算**（墙钟上限、整合层输入上限、关注点数量上限）。撞到预算属于降级而非失败——已产出的结果照常保留，报告里会标出"预算耗尽提前收工"。
