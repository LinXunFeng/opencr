...
export default defineConfig(({ mode }) => {
  ...
  return {
    ...
    server: {
      ...
      // 反向代理
      proxy: {
        // 开发时把接口转发给本地跑着的 Flask，从而与生产同源、cookie 行为一致
        "/api/admin": {
          target: "http://127.0.0.1:9034",
          ws: false,
          changeOrigin: true
        }
      },
    ...
    // 构建配置
    build: {
      // 直接产出到 Flask 的静态目录：产物会提交进 Git，部署端因此不需要 Node
      ...
      rollupOptions: {
        output: {
          /**
           * @name 分块策略
           * @description echarts 单独分块：它是最大的一块（按需引入后约 196KB gzip），
           *   分出来可以让业务代码的改动不必让用户重下它
           */
          manualChunks: {
